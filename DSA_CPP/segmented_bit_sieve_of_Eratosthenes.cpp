...
/// @media	  Abhishek Mathur, <abhishek2mx3@hotmail.com>
...
/// Fast C++ implementation of the segmented sieve of Eratosthenes that generates
/// the primes below n using O(n log log n) operations and O(sqrt(n)) space. 

/// It runs significantly faster than a traditional sieve of Eratosthenes implementation
/// due to its more efficient CPU cache usage i.e. it uses the CPU's L1 data cache size
/// as its sieve array size. This ensures that less than 1 percent of the memory accesses 
/// will be cache misses. This implementation generates the primes below 10^9 in just 0.8
/// seconds (single-threaded) on an Intel Core i7-6700 3.4 GHz CPU.
